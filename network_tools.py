...
def network_draw(A, labels_dict, node_color='red',node_size=1000,font_size=17,weighted=False,file_name='graph_weighted.png'):
...
#############################################
# network_directed: Create and plot undirected graph from a symmetric adjacency matrix, A.
# Similar to network_draw

def network_directed(A,labels,node_size=1000,node_color='blue',node_shape='circle',fill_color='yellow',node_font=14,show_weights=False,edge_font=14,edge_color='black',arrow_size=1,file_name='graph.eps'):                
    ...
    #nx.set_node_attributes(G,node_size=nodesize,node_font=node_font)
    if show_weights:
    ...
    D.edge_attr.update(color=edge_color,arrowsize=arrow_size,splines='curved')
    pos = D.layout('dot')
    D.draw(file_name)
    file_name_png=file_name.split('.')[0]+'.png'
    D.draw(file_name_png)
    return G
...
# Returns: Undirected graph G and adjacency matrix A
    
def network_from_edgelist(filename):
    b=npy.loadtxt(filename)
    dim=0
    for r in b:
        dim=dim+1
    A=npy.zeros((dim,dim))
    for r in b:
        A[int(r[0])-1][int(r[1])-1 ]=r[2]
    G=nx.from_numpy_matrix(A)
    return G, A
...
def network_eigenvector_centrality(G, labels_dict,node_color='red',node_size=1000,font_size=17):
...
def louvain(G,labels_dict,node_size=1000,font_size=17):
...
def leiden(G,labels_dict,node_size=1000,font_size=17,resolution_parameter=.5):
...
def Laplacian(G):
    ...
    L=L.toarray()
    x=[i for i in range(1,len(spectrum)+1)]
...
def spectral_communities2(G,k,labels_dict,node_size=1000,font_size=17,filename='spectral_communities.png'):
    labels=list(labels_dict.values())
    np.random.seed(0)
...
def spectral_communities(G,k,labels_dict,node_size=1000,font_size=17,filename='spectral_communities.png'):
  np.random.seed(0)
  sc = SpectralClustering(k, affinity='precomputed', n_init=100)
...
# Fuzzy c-means clustering based upon eigenvectors of # of k smallest eigenvalues of the Laplacian L corresponding to a graph. 
